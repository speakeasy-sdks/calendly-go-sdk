// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
)

// SubmissionEventTypeResultTypeEnum - Indicates that the routing form submission resulted in a redirect to an event type booking page.
type SubmissionEventTypeResultTypeEnum string

const (
	SubmissionEventTypeResultTypeEnumEventType SubmissionEventTypeResultTypeEnum = "event_type"
)

func (e *SubmissionEventTypeResultTypeEnum) UnmarshalJSON(data []byte) error {
	var s string
	if err := json.Unmarshal(data, &s); err != nil {
		return err
	}
	switch s {
	case "event_type":
		*e = SubmissionEventTypeResultTypeEnum(s)
		return nil
	default:
		return fmt.Errorf("invalid value for SubmissionEventTypeResultTypeEnum: %s", s)
	}
}

// SubmissionEventTypeResult - Information about the event type Routing Form Submission result.
type SubmissionEventTypeResult struct {
	// Indicates that the routing form submission resulted in a redirect to an event type booking page.
	Type SubmissionEventTypeResultTypeEnum `json:"type"`
	// A reference to the event type resource.
	Value string `json:"value"`
}
